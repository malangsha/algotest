...
class DataManager:
    ...
    def subscribe_to_timeframe(self, strategy_id: str, instrument: Instrument, timeframe: str = 'tick'):
        """
        ...
        with self.data_lock:
            ...
            self.timeframe_subscriptions[timeframe][strategy_id].add(instrument.symbol)

            self.logger.info(f"Strategy {strategy_id} subscribed to {instrument.symbol} data on {timeframe} timeframe")
            return True
        ...
        try:
            ...
            if isinstance(symbol, str):
                ...
                instrument = Instrument(
                    symbol=symbol,
                    exchange=exchange,
                    instrument_type=InstrumentType.INDEX if symbol in ['NIFTY INDEX', 'NIFTY BANK'] else InstrumentType.EQUITY,
                    asset_class=AssetClass.INDEX if symbol in ['NIFTY INDEX', 'NIFTY BANK'] else AssetClass.EQUITY,
            ...
            # # If we have a broker with subscribe capability, use it
            # if self.broker and hasattr(self.broker, 'subscribe'):
            #     return self.broker.subscribe(instrument.symbol, instrument.exchange)
            ...
            # No subscription mechanism available, but return success so we don't break
            # other components since we're still tracking the symbol internally
